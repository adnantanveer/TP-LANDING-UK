...
function mountScrollWorld(container, config) {
  ...
  function layout() {
    ...
    // Segment coordinates are absolute document-scroll positions, anchored to
    // this container's own top — not always 0. That's what lets more than one
    // mountScrollWorld() live on the same page (a hero instance at the top
    // plus a second one further down): each reads window.scrollY directly
    // against its own segment start/end, so without this offset a second
    // instance would assume its content starts at the very top of the page.
    // scrollY + getBoundingClientRect().top is the standard absolute-offset
    // formula — it's correct at any current scroll position, not just at load.
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      let cop;
      // "Greets on landing" assumes section 0 starts at document position 0,
      // so pr clamps to 0 (full opacity) only right at y=0 — true for a
      // hero at the top of the page. An embedded instance mounted further
      // down has seg.start > 0, and pr clamps to 0 for the entire stretch
      // BEFORE it too, which without the `before` guard here showed its
      // copy at full opacity from page load, long before its own scroll
      // range began. before is always false at y=0 for a real top-of-page
      ...
      // scroll frame, so it fully REPLACES the CSS rule's `translateY(-50%)`
      // vertical-centering instead of composing with it — the element ends up
      // top-anchored at the viewport's 50% line instead of centered on it.
      // Harmless for short copy blocks, but a tall one (long title + body +
      // tags + a .sw-copy__cta row, e.g. the last section) then runs past the
      // bottom of the viewport and clips its own CTA buttons. Folding the
      // -50% back into the calc() keeps the intended centering and the small
      // ±2vh settle-in drift together, composed rather than clobbered — same
      // fix family as the SVG transform note in CLAUDE.md.
